...
describe "TerminalNotifier::Guard" do
  describe ".execute" do
    ...
    it "executes with the right icon path according to the type option" do
      command = [TerminalNotifier::Guard.bin_path, '-message', 'ZOMG', '-appIcon', TerminalNotifier::Guard::GUARD_ICON, '-contentImage', 'https://github.com/lanej0/terminal-notifier-guard/raw/master/icons/Success.icns']
      if RUBY_VERSION < '1.9'
        require 'shellwords'
        command = Shellwords.shelljoin(command)
      end
  ...
  describe ".failed" do
    it "executes with the 'failed' icon flag" do
      command = [TerminalNotifier::Guard.bin_path, '-message', 'ZOMG', '-appIcon', TerminalNotifier::Guard::GUARD_ICON, '-contentImage', 'https://github.com/lanej0/terminal-notifier-guard/raw/master/icons/Failed.icns']
      if RUBY_VERSION < '1.9'
        require 'shellwords'
        command = Shellwords.shelljoin(command)
      end
  ...
  describe ".pending" do
    it "executes with the 'pending' icon flag" do
      command = [TerminalNotifier::Guard.bin_path, '-message', 'ZOMG', '-appIcon', TerminalNotifier::Guard::GUARD_ICON, '-contentImage', 'https://github.com/lanej0/terminal-notifier-guard/raw/master/icons/Pending.icns']
      if RUBY_VERSION < '1.9'
        require 'shellwords'
        command = Shellwords.shelljoin(command)
      end
